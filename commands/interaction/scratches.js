const { SlashCommandBuilder, EmbedBuilder } = require('discord.js');
const scratch = require('./scratch.json');

module.exports = {
  data: new SlashCommandBuilder()
    .setName('scratches')
    .setDescription('Scratches target user with a fun GIF!')
    .addUserOption(option =>
      option.setName('target')
        .setDescription('The user to scratch (leave blank to scratch yourself)')
        .setRequired(false)
    ),

  async execute(interaction) {
    const targetUser = interaction.options.getUser('target') || interaction.user;
    const author = interaction.user;
    

    const randomIndex = Math.floor(Math.random() * scratch.length);
    const { image: scratchImage, number: scratchNumber } = scratch[randomIndex];

    const scratchEmbed = new EmbedBuilder()
      .setColor('#E0FBFF')
      .setImage(scratchImage);

    if (targetUser.id === author.id) {
      scratchEmbed
        .setAuthor({ name: `${author.username} just scratched themselves..? weirdo`, iconURL: author.displayAvatarURL() })
        .setFooter({ text: `${author.username} just scratched themselves\n${scratchNumber}`, iconURL: author.displayAvatarURL() });
    } else if (targetUser.id === interaction.client.user.id) {
      scratchEmbed
        .setAuthor({ name: `That feels nice... ${author.username}, It's relaxing`, iconURL: author.displayAvatarURL() })
        .setFooter({ text: `I wouldn't mind some scratches once in a while...\n${scratchNumber}`, iconURL: author.displayAvatarURL() });
    } else {
      scratchEmbed
        .setAuthor({ name: `${author.username} decided to scratch ${targetUser.username}, they look like they enjoyed it`, iconURL: author.displayAvatarURL() })
        .setFooter({ text: `${author.username} just scratched ${targetUser.username}. They seem happy!\n${scratchNumber}`, iconURL: targetUser.displayAvatarURL() });
    }

    await interaction.reply({ embeds: [scratchEmbed] });
  },
};